...
class AmbilSampahPage extends StatelessWidget {
  const AmbilSampahPage({super.key});

  @override
  Widget build(BuildContext context) {
    return AmbilSampahPageFull();
  }
}

class AmbilSampahPageFull extends StatefulWidget {
  const AmbilSampahPageFull({super.key});
  
  

  @override
  State<StatefulWidget> createState() => _AmbilSampahPageFull();

}



class _AmbilSampahPageFull extends State<AmbilSampahPageFull> {
  ...
  String uid_user = '';
  String uid_distributor = '';
  String formattedDate = '';
  String daerahKerja = '';
  String alamat_user = '';
  double berat_distributor = 0.0;
  ...
  try {
    ...
    if (arguments != null) {
      setState(() {
        ...
        uid_distributor = arguments['uid_distributor'];
...
Future<void> updateLaporanDocument() async {
  ...
  try {
    ...
    await firestore.doc(docPath).update({
      'berat_distributor': berat_distributor,
      'status_laporan': 'perjalanan_pemilah',
      'uid_distributor': uid_distributor,
      'tanggal_waktu_ambil': DateTime.now(), // Menambahkan tanggal_waktu_ambil
    });
  ...
  try {
    ...
    final compressedImageData = await compressImage(image);
      ...
      String modifiedPath = basePath.replaceFirst('akun/akun', 'laporan_distributor') + "$dateFolder";
                            ...
                            onPressed: () async {
                              ...
                              builder: (context) => AlertDialog(
                                ...
                                content: Column(
                                  ...
                                  children: [
                                    Text('Sampah anda akan diambil sesuai dengan jam yang anda pilih, yakin ingin diambil?',
                                    style: TextStyle(color: Colors.white),),
                                    SizedBox(height: 16.0),
                                    InputLayout('Berat', TextFormField(
                                      onChanged: (String value) => setState(() {
                                        berat_distributor = double.tryParse(value) ?? 0.0;
                                      }),
                                          ...
                                          return AlertDialog(
                                            ...
                                            actions: [
                                              TextButton(
                                                child: Text("OK"),
                                                onPressed: () {
                                                  Navigator.pushNamedAndRemoveUntil(context, '/list_alamat_ambil', ModalRoute.withName('/list_tanggal_ambil'),
                                                  arguments: {
